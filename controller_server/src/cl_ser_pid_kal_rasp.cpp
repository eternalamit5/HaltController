...
void *serverfunc(void*){
	...
	serv_addr.sin_addr.s_addr = INADDR_ANY; //Server bound to this address(127.0.0.1)
...
void *clientfunc(void*){
	...
	struct sockaddr_in	servaddr0, servaddr1, servaddr2;
	...
	//----------Server0 (Loopback) details---------------
	memset(&servaddr0, 0, sizeof(servaddr0));
	servaddr0.sin_family = AF_INET;
	servaddr0.sin_port = htons(CPUc_Gs_PORT);
	servaddr0.sin_addr.s_addr = inet_addr("127.0.0.1");

	//----------Sever1 details---------------
	memset(&servaddr1, 0, sizeof(servaddr1));
	servaddr1.sin_family = AF_INET;
	servaddr1.sin_port = htons(CPUc_Gs_PORT);
	servaddr1.sin_addr.s_addr = inet_addr("169.254.102.146");
	...
	servaddr2.sin_addr.s_addr = inet_addr("169.254.102.146");
	...
	while(1){
		...
		// Sending data to Server0
		correction_str = "$KALMAN1:" + to_string(global_x_u) + ":" + to_string(global_y_u);
		correction_data = const_cast<char*>(correction_str.c_str());
		sendto(sockfd, (const char *)correction_data, strlen(correction_data),MSG_CONFIRM, (const struct sockaddr *)&servaddr0, sizeof(servaddr0));
